...
public class MyPickerActivity extends AppCompatActivity implements DataCallback, View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(com.dmcbig.mediapicker.R.layout.main);
        recyclerView = (RecyclerView) findViewById(com.dmcbig.mediapicker.R.id.recycler_view);
        findViewById(com.dmcbig.mediapicker.R.id.btn_back).setOnClickListener(this);
        setTitleBar();
        done = (Button) findViewById(com.dmcbig.mediapicker.R.id.done);
        category_btn = (Button) findViewById(com.dmcbig.mediapicker.R.id.category_btn);
        preview = (Button) findViewById(com.dmcbig.mediapicker.R.id.preview);
    ...
    public void setTitleBar() {
        ...
        if (type == PickerConfig.PICKER_IMAGE_VIDEO) {
            ((TextView) findViewById(com.dmcbig.mediapicker.R.id.bar_title)).setText(getString(com.dmcbig.mediapicker.R.string.select_title));
        } else if (type == PickerConfig.PICKER_IMAGE) {
            ((TextView) findViewById(com.dmcbig.mediapicker.R.id.bar_title)).setText(getString(com.dmcbig.mediapicker.R.string.select_image_title));
        } else if (type == PickerConfig.PICKER_VIDEO) {
            ((TextView) findViewById(com.dmcbig.mediapicker.R.id.bar_title)).setText(getString(com.dmcbig.mediapicker.R.string.select_video_title));
        }
    }
    ...
    void createFolderAdapter() {
        ...
        mFolderPopupWindow.setAnchorView(findViewById(com.dmcbig.mediapicker.R.id.footer));
        ...
        mFolderPopupWindow.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                Intent i = new Intent(MyPickerActivity.this, GlideListActivity.class);
                i.putParcelableArrayListExtra("list", mFolderAdapter.getSelectMedias());
                startActivity(i);
            }
        });
    }
    ...
    void getMediaData() {
        if (EasyPermissions.hasPermissions(this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
            ...
            EasyPermissions.requestPermissions(this, getString(com.dmcbig.mediapicker.R.string.READ_EXTERNAL_STORAGE), 119, Manifest.permission.READ_EXTERNAL_STORAGE);
        }
    }
    ...
    void setButtonText() {
        ...
        done.setText(getString(com.dmcbig.mediapicker.R.string.done) + "(" + gridAdapter.getSelectMedias().size() + "/" + max + ")");
        preview.setText(getString(com.dmcbig.mediapicker.R.string.preview) + "(" + gridAdapter.getSelectMedias().size() + ")");
    }

    @Override
    public void onClick(View v) {
        int id = v.getId();
        if (id == com.dmcbig.mediapicker.R.id.btn_back) {
            finish();
        } else if (id == com.dmcbig.mediapicker.R.id.category_btn) {
            if (mFolderPopupWindow.isShowing()) {
                mFolderPopupWindow.dismiss();
            } else {
                mFolderPopupWindow.show();
            }
        } else if (id == com.dmcbig.mediapicker.R.id.done) {
            done(gridAdapter.getSelectMedias());
        } else if (id == com.dmcbig.mediapicker.R.id.preview) {
            if (gridAdapter.getSelectMedias().size() <= 0) {
                Toast.makeText(this, getString(com.dmcbig.mediapicker.R.string.select_null), Toast.LENGTH_SHORT).show();
                return;
            }
