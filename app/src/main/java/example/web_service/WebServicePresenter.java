...
import com.style.base.BasePresenter;
...
public class WebServicePresenter extends BasePresenter {

    MutableLiveData<String> content = new MutableLiveData<>();

    private WebServiceView mView;

    public WebServicePresenter(WebServiceView v) {
        mView = v;
    }
    ...
    public void getWeather(String code) {
        Disposable d = getHttpApi().getWeather(code).subscribe(s -> mView.setWeather(s));
        addTask(d);
    }
